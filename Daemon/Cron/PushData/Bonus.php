<?php
/**
 * 积分兑红包日志 rsync 推送兑红包订单
 * User: cts(haoman@staff.weibo.com)
 * Date: 2019/2/28
 * 第一次推全量，以后每天推增量（前一天的创建订单or前一天有更新的订单）
 * 提案：http://rt.intra.sina.com.cn/issue/view?issue_id=343538
 */
...
class Daemon_Cron_PushData_Bonus extends Daemon_Abstract
{
    protected $cronName = 'PUSHDATA_BONUS';
    ...
    public function run()
    {
        ...
        try {
            ...
            //数据存放根目录
            $base_path = "{$_SERVER['SINASRV_DATA_DIR']}bonus";
            //当天推送地址日期存放
            ...
            $file = $target_path."exchange_bonus_".$yesterday.".txt";

            if(file_exists($file)) {
                exit('file '.$file.' exists');
            }

            $sql = "select * from ex_money_order where (`create_time` >= '{$start}' and `create_time` <= '{$end}') or (`update_time` >= '{$start}' and `update_time` <= '{$end}') ";
            ...
            Tool_Mail::sendMail("积分兑红包日志生成成功file:{$file}",$users);
...
new Daemon_Cron_PushData_Bonus();  //注意和类名保持一致