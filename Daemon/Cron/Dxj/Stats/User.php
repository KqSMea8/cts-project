 ...
 * 日报,旧用户
 */
class Daemon_Cron_Dxj_Stats_User extends Daemon_Abstract
{
    protected $start;
    protected $end;
    protected $date;
    ...
    public function run()
    {
        ...
        $this->where_paytime = "`status` >= ".Model_Const::MONEY_ORDER_STATUS_PAY." and create_time>='{$this->start}' and create_time<='{$this->end}'";

        $redis_w = new Lib_Redis(self::DBNAME, Lib_Redis::MODE_WRITE);
        $db = new Lib_Mysql(self::DBNAME);
        $key_t = 'exchange_bonus_stat_str_new_users'.date('Ymd', strtotime('-1 days'));//当天新用户总数

        $sql = "select uid from `ex_money_order` where `status` >= ".Model_Const::MONEY_ORDER_STATUS_PAY." and `create_time`<'{$this->start}'  group by uid;"; //全量的用户集合
        $ret = $db->fetchAll($sql);
...
new Daemon_Cron_Dxj_Stats_User();
