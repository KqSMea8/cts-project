...
class Daemon_Cron_Stats_Daily extends Daemon_Abstract
{
    protected $start;
    protected $end;
    protected $date;
    ...
    public function run()
    {
        ...
        $this->csv_path = "{$_SERVER['SINASRV_DATA_DIR']}exchange_daily_{$this->date}.csv";
        ...
        //累计 start
        $txt .= '累计兑换用户:' . $hist['order_users'] .PHP_EOL;
        $txt .= '累计兑换订单:' . $hist['order_count'] .PHP_EOL;
        $txt .= '累计兑换积分:' . $hist['order_score'] .PHP_EOL;
        $txt .= '累计人均兑换次数:' . $hist['order_average'] .PHP_EOL;
        //累计 end
    ...
    private function sendMail($content){
        ...
        while(1){
            $log_ret = Tool_Mail::sendMailWithAtta($content, $users, date('Ymd') . '报表', false, 'daily_notice', array(array('path' => $this->csv_path, 'name' => 'exchange_daily_'.$this->date.'.csv')));
            var_dump($log_ret);
    ...
    private function getPvUv() {
        ...
        //pv
        $sqlPv = "select count(*) as pvs from {$table} where `action` = 'view_index' and `date` = '{$this->date}'";
        ...
        //uv
        $sqlUv = "select count(DISTINCT(`uid`)) as uvs from {$table} where `action` = 'view_index' and `date` = '{$this->date}'";
    ...
    private function getOrderTotal() {
        ...
        //总订单数
        $sql2 = "select count(*) as total from `ex_product_order` where `product_id` not in('1543568453100017','11543549070100010','11543547500100004','11543489045100000') and `order_id` not in ('100115','100116','101086') and `create_time` >= '{$this->start}' and `create_time` <= '{$this->end}' ";
        ...
        //已付款订单
        $sql3 = "select count(*) as total from `ex_product_order` where `product_id` not in('1543568453100017','11543549070100010','11543547500100004','11543489045100000') and `order_id` not in ('100115','100116','101086') and `status` = ".Model_Const::ORDER_STATUS_PAY." and `create_time` >= '{$this->start}' and `create_time` <= '{$this->end}' ";
        ...
        //未付款订单
        $sql4 = "select count(*) as total from `ex_product_order` where `product_id` not in('1543568453100017','11543549070100010','11543547500100004','11543489045100000') and `order_id` not in ('100115','100116','101086') and `status` = ".Model_Const::ORDER_STATUS_CREATE." and `create_time` >= '{$this->start}' and `create_time` <= '{$this->end}' ";
        ...
        //已发货订单
        $sql5= "select count(*) as total from `ex_product_order` where `product_id` not in('1543568453100017','11543549070100010','11543547500100004','11543489045100000') and `order_id` not in ('100115','100116','101086') and `status` = ".Model_Const::ORDER_STATUS_PAY." and `delivery_status` = ".Model_Const::ORDER_DELIVERY_STATUS_SEND."  and `create_time` >= '{$this->start}' and `create_time` <= '{$this->end}' ";
        ...
        //待发货订单
        $sql6= "select count(*) as total from `ex_product_order` where `product_id` not in('1543568453100017','11543549070100010','11543547500100004','11543489045100000') and `order_id` not in ('100115','100116','101086') and `status` = ".Model_Const::ORDER_STATUS_PAY." and `delivery_status` = ".Model_Const::ORDER_DELIVERY_STATUS_ADDRESS."  and `create_time` >= '{$this->start}' and `create_time` <= '{$this->end}' ";
    ...
    private function getHistoryData(){
        ...
        $key_t = 'exchange_stat_str_history_data';//累计
    ...
    private function getNewUser($date){
        $redis_r = new Lib_Redis(self::DBNAME, Lib_Redis::MODE_WRITE);
        $key_t = 'exchange_stat_str_new_users'.$date;//当天新用户总数
        $ret = $redis_r->get($key_t);
        $ret = json_decode($ret, true);
        return $ret;
    }
