 ...
 * 日报,旧用户
 */
class Daemon_Cron_Stats_Week_Users extends Daemon_Abstract
{
    protected $start;
    protected $end;
    protected $date;
    ...
    public function run()
    {
        ...
        $this->where_paytime = "`status` = ".Model_Const::ORDER_STATUS_PAY." and create_time>='{$this->start} 00:00:00' and create_time<='{$this->end} 23:59:59'";

        $redis_w = new Lib_Redis(self::DBNAME, Lib_Redis::MODE_WRITE);
        $db = new Lib_Mysql(self::DBNAME);
        $key_t = 'exchange_stat_str_new_users_week'.date('Ymd', strtotime('-1 days'));//当天新用户总数

        $sql = "select uid from `ex_product_order` where `status` = ".Model_Const::ORDER_STATUS_PAY." and `create_time`<'{$this->start} 00:00:00'  group by uid;"; //全量的用户集合
        $ret = $db->fetchAll($sql);
        ...
        $sqlNew = "select * from `ex_product_order` where {$this->where_paytime} ";//当天的用户集合
        $new_paid_count = 0;
...
new Daemon_Cron_Stats_Week_Users();
