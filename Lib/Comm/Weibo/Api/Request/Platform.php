...
class Comm_Weibo_Api_Request_Platform extends Comm_Weibo_Api_Request_Abstract {
     ...
     */
    public function __construct($url, $method = false,$key = '') {
...
//          }
        ...
        foreach($_COOKIE as $name=>$value){
            $this->http_request->add_cookie($name, $value);
        }        
     ...
     */
     public function get_rst($throw_exp = TRUE,$defaut = array()) {
        //使用Tauth2认证方式
        $tauth_uid = Comm_Context::get('tauth_uid', "");
        $tauth_auth_string = Comm_Context::get('tauth_auth_string', "");

        if (!empty($tauth_uid) && !empty($tauth_auth_string) && $this->usepsd == false) {
            $url_encode = $this->http_request->urlencode;
            $this->http_request->urlencode = false;
            $this->http_request->add_header("Authorization", $tauth_auth_string, false);
            $this->http_request->urlencode = $url_encode;
        }

        //$platform_app_token = Model_Remind::getPlatformAppToken();
         $platform_app_token = Data_Etc::getAppToken(Model_Const::APP_SOURCE_JINGCAI);
         $platform_app_token = json_decode($platform_app_token, true);
         $platform_app_token = $platform_app_token['tauth_token'];
        $this->http_request->add_header("PlatformAppToken", $platform_app_token, false);
        parent::send();
     ...
     * @return 接口无异常时的正常返回值
     */
     public function get_rst_detail($throw_exp = TRUE,$defaut = array()) {
        //使用Tauth2认证方式
        //$token_res = Model_Remind::getTauth();
        $token_res = Data_Etc::getAppToken(Model_Const::APP_SOURCE_JINGCAI);
        $token_res = json_decode($token_res, true);
        $token_res = $token_res['tauth_token'];
        $uid = Comm_Util::conf("Env.platform_api_source_duobao");
        $token_arr = $token_res;
        $uid_str ="uid=".$uid;
        $sign = urlencode(base64_encode(hash_hmac('sha1',$uid_str,$token_arr['tauth_token_secret'],true)));
        $tauth_str = "TAuth2 token=\"".urlencode($token_arr['tauth_token'])."\",param=\"".urlencode($uid_str)."\",sign=\"$sign\"";
        Comm_Context::clear();
        Comm_Context::set('tauth_uid', $uid);
        Comm_Context::set('tauth_auth_string', $tauth_str);
        ...
        $this->http_request->cookies = null;
        parent::send();
        $content = $this->http_request->get_response_content();
        $result = Comm_Util::json_decode($content, true);
