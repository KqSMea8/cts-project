...
class Lib_Validation_Validator
{
    ...
    {
        if (empty($ruleConfig) || !is_array($ruleConfig)) {
            return true;
        }
        $this->parseRule($ruleConfig);
        foreach ($this->ruleConfig as $paramName => $rule) {
            $checkClass = "Lib_Validation_Validator_" . ucfirst(strtolower($rule['checkType']));
            //echo $checkClass;
    ...
    {
        $customTip = $this->getCustomTip($paramName);
        $msg = !empty($customTip) ? $customTip : (!empty($msg) ? $msg : "param " . $paramName . " is error!");
