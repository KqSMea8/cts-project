...
class Tool_Saetoauthv2
{
    ...
    function parseSignedRequest($signed_request) {
        ...
        $sig =  str_replace("=", "", strtr(base64_encode(hash_hmac('sha256', $payload, $this->client_secret, true)), '+/', '-_'));
        return ($encoded_sig !== $sig)? '-2':$data;
    }
    ...
    function json_decode_fix($response, $assoc){
        $response = preg_replace ( '/("id":)(\d{9,})/i', '${1}"${2}"', $response );
        $response = preg_replace ( '/("next_cursor":)(\d{9,})/i', '${1}"${2}"', $response );
    ...
    function get($url, $parameters = array())
    {
        ...
        if ($this->format === 'json' && $this->decode_json)
        {
            ...
            return $this->json_decode_fix($response, true);
        }
        return $response;
    }
    ...
    function http($url, $method, $postfields = NULL, $headers = array())
    {
		...
		$set_cookies[] = array('SUE', $_COOKIE['SUE']);
		$set_cookies[] = array('SUP', $_COOKIE['SUP']);
		$cookies = array();
		foreach ($set_cookies as $var) {
			$cookies[] = $var[0].'='.rawurlencode($var[1]);
		}
		curl_setopt($ci, CURLOPT_COOKIE, implode(';', $cookies));
...
class SaeTClientV2
{
     ...
     * @return array
     */
    function queryid($mid, $type = 1, $is_batch = 0, $inbox = 0, $isBase62 = 0, $source)
    {
        $params = array();
        $params['mid'] = $mid;
     ...
     * @return array
     */
    function show_user_by_id($uid, $source = null)
    {
        $params = array();
        if ($uid !== NULL)
        {
            $this->id_format($uid);
            $params['uid'] = $uid;
            $params['source'] = $source ? $source : $this->oauth->client_id;
        }
