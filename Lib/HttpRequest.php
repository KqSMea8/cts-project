...
class Lib_HttpRequest {
    const CRLF = "\r\n";
    const CHECKURL  = 'http://www.sina.com.cn/';
    ...
    public function curl_init() {
        ...
        $ch = Lib_HttpRequestPool::get_curl($this->get_host_id());
    ...
    public function send() {
        ...
        if (curl_errno($this->ch) === 0) {
            $rtn = true;
            $this->set_response_state(true, "");
        } else {
            $this->set_response_state(false, curl_error($this->ch));
            $rtn = false;
        }
        $this->set_response($content, curl_getinfo($this->ch));

        Lib_HttpRequestPool::reset_curl_state($this->get_host_id(), $this->get_curl_id());
        $this->reset_ch();
    ...
    public function set_response_state($state, $error_msg) {
    ...
    private function curl_setopt() {
        ...
        if ($version["version"] < "7.16.2") {
            curl_setopt($this->ch, CURLOPT_CONNECTTIMEOUT, round($this->connect_timeout / 1000, 3));
            curl_setopt($this->ch, CURLOPT_TIMEOUT, round($this->timeout / 1000, 3));
        } else {
...
//    }
     ...
     */
    public function varify_url(){
        $check = @fopen(self::CHECKURL, "r");
        if ($check) {
            $status = true;
        }else {
            $status = false;
