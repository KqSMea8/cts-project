...
class Lib_HttpRequestPool {
    ...
    public static function attach(Lib_HttpRequest $http_request) {
    ...
    public static function send($force_all = false) {
        if (empty(self::$request_pool)) {
            throw new Lib_Exception_Program('request pool is empty');
        }
        ...
        do {
            ...
            while ($rst = curl_multi_info_read(self::$mh, $queue_point)) {
                $curl_id = Lib_HttpRequest::fetch_curl_id($rst['handle']);
                ...
                if ($rst['result'] == CURLE_OK) {
                    ...
                    if (!$force_all) {
                        ...
                        throw new Lib_Exception_Program("Request " . $request->url . ": " . $error_msg);
                    }
                }
            }
    ...
    private static function get_curl_create($host_id) {
        $ch = curl_init();
        $curl_id = Lib_HttpRequest::fetch_curl_id($ch);
